...
const STYLE_LIBRARY = [
  ...
  {
    id: 'warm-editorial-minimal',
    name: 'Warm Editorial Minimal',
    category: 'full-site',
    aesthetics: ['minimal', 'editorial', 'clean', 'premium', 'warm', 'approachable', 'boutique', 'calm'],
    industries: ['design', 'studio', 'consulting', 'editorial', 'publishing', 'architecture', 'interiors', 'wellness', 'beauty', 'lifestyle', 'saas', 'portfolio', 'nonprofit', 'education', 'food'],
    fonts: "A quiet grotesque (Neue Haas / Inter / Söhne feel) for body, paired with ONE characterful display (a high-contrast serif or a wide grotesque) used large but sparingly; an optional small utility/mono face for captions and data labels.",
    dna: {
      mood: 'Quiet, warm, paper-like editorial calm. Confidence comes from restraint and typographic contrast — never from effects. It reads like a beautifully set print magazine translated to screen: flat, airy, considered, expensive in its stillness.',
      typography: 'Typographic CONTRAST is the primary device — a large, characterful display set against a small, quiet grotesque body. Generous leading, intentional weights and widths, a clear 3-tier scale. Because colour and shadow are stripped back, the type carries the entire personality.',
      color: 'Warm monochrome. A paper / oatmeal off-white base (NEVER pure white), soft warm ink (a warm near-black, never #000), and ONE or TWO MUTED PASTEL accents (dusty sage, clay, sky, ochre) at low saturation used only on small fills, tags and links. No gradients anywhere. No vivid colour.',
      layout: 'Flat BENTO GRIDS — modular rounded rectangles of varying sizes tiled edge-to-edge, each cell holding exactly one idea (a stat, an image, a quote, a feature). Wide margins, generous whitespace, a strong baseline grid, asymmetry inside order. No cards nested inside cards.',
      motion: 'Minimal and physical — gentle fades and small (8–12px) position shifts on scroll-in, once; soft hover lifts with NO shadow bloom. Nothing bounces or zooms. The stillness is the design; over-animating would break it.',
      imagery: 'Soft, warm, natural-light photography OR simple flat / line illustration, all sharing one muted grade and the same tile border-radius. No drop shadows, no glows, no glossy stock. The image sits flat inside its bento cell.',
      signature: 'The flat warm-monochrome bento grid + the big-display-vs-quiet-body typographic contrast + one or two muted pastel accents, with ZERO gradients and zero heavy shadows — depth comes from spacing and tiling, not effects.',
    },
    techniques: [
      'Flat bento grid (pure CSS grid): modular tiles of mixed col-span / row-span on a uniform grid, a single consistent gap, and one shared border-radius (≈20–28px); each tile holds one idea. No nested cards.',
      'Typographic-contrast system: one large characterful display (used big but sparingly) against a small quiet grotesque body; hierarchy comes from size and weight, never from colour. Generous leading; balanced headlines (text-wrap: balance).',
      'Warm-monochrome palette discipline: oatmeal/paper base (~#F6F3EC, never #FFF), warm ink (~#1C1A17, never #000), and 1–2 desaturated pastel accents on tags/links/small fills only. Strictly no gradients.',
      'Depth via SPACING, not shadows: separation comes from whitespace, hairline borders and the bento tiling. Avoid drop-shadows and glows entirely — at most a flat tonal tile or a 1px warm border.',
      'Quiet motion: fade + small rise on scroll-in (once), soft hover lift with no shadow bloom; honour prefers-reduced-motion. No bounce, no parallax, no auto-playing carousels.',
      'One consistent image grade: every photo shares a warm, slightly desaturated treatment and the tile radius; flat illustration is an acceptable alternative but never mix glossy stock with the muted grade.',
    ],
    imagePrompts: [
      'warm natural-light editorial photograph, soft muted grade, oatmeal and clay tones, calm composition with negative space, magazine quality, sits flat inside a bento tile with no drop shadow',
      'minimal flat line illustration in a single muted pastel on a warm paper background, simple geometric forms, quiet and editorial, no gradient',
      'soft understated still life on a warm off-white surface, gentle natural shadow, desaturated palette, premium product shot for a flat bento grid cell',
    ],
    source: 'skill',
  },
  {
    ...
    category: 'full-site',
    aesthetics: ['bold', 'brutalist', 'editorial', 'techy', 'modern', 'disruptor', 'dark', 'industrial', 'utilitarian'],
    industries: ['tech', 'crypto', 'web3', 'ai', 'data', 'security', 'developer-tools', 'saas', 'media', 'music', 'fashion', 'streetwear', 'gaming', 'agency', 'engineering', 'editorial'],
    fonts: "A Swiss grotesque (Helvetica / Neue Haas / Akzidenz feel) for display at extreme scale, plus a MONOSPACE (terminal) face for every label, data point, coordinate, timestamp and caption. Optionally a condensed grotesque for stamped headlines.",
    dna: {
      mood: 'Raw, mechanical, declassified-blueprint intensity — Swiss typographic print collides with a military / terminal readout. Utilitarian, dense, confident, faintly dystopian. Nothing is soft; everything looks engineered, stamped, scanned, or printed.',
      typography: 'EXTREME type-scale contrast — enormous tight-tracked uppercase grotesque headlines against tiny monospace metadata. Hard left alignment to a rigid grid. Monospace carries all labels, data, coordinates and timestamps. Type is structural, not decorative.',
      color: 'Utilitarian and restrained — raw concrete/paper (off-white or warm grey) OR a near-black terminal base, with ink black, and ONE hazard/utility accent (signal red, safety orange, terminal green, or hi-vis yellow) used like a painted warning stripe. Mostly monochrome; the accent is a flag, never decoration.',
      layout: 'A rigid, EXPOSED grid — visible 1px column/row rules, boxed cells, ledgers and tables, hard edges, zero or minimal border-radius. Dense, information-rich, multi-column. Section numbers, coordinates and timestamps stamped into the margins.',
      motion: 'Mechanical and abrupt rather than smooth — snap / stepped transitions, a monospace marquee ticker, counters that tick, a blinking cursor, slight glitch or jitter on hover. Motion reads like a machine readout, not an easing curve.',
      imagery: 'High-contrast black-and-white or duotone photography, technical diagrams and schematics, halftone / dithered images, plus ANALOG DEGRADATION — grain, scanlines, photocopy/print artifacts, registration marks. Everything looks printed, scanned, or rendered on a terminal.',
      signature: 'The exposed rigid grid with hairline rules + extreme grotesque-vs-monospace type contrast + a single hazard accent + analog degradation (grain / scanlines / print marks) that make the whole thing read as a declassified blueprint or terminal readout.',
    },
    techniques: [
      'Exposed grid system: show the grid — visible 1px column/row rules, boxed cells, ledger/table layouts, hard left alignment, near-zero border-radius. The structure is the decoration.',
      'Extreme type-scale contrast: giant tight-tracked uppercase grotesque headlines (huge clamp) beside 11–13px monospace labels; use monospace for all data, coordinates, timestamps and captions.',
      'Single hazard accent: keep everything mono (ink on concrete, or terminal-green on near-black) and deploy ONE utility colour (signal red / safety orange / hi-vis yellow / terminal green) only on warnings, key data, and the primary action.',
      'Analog degradation (zero-dependency): a CSS/SVG grain or noise overlay, scanlines via repeating-linear-gradient, halftone/dither on images, plus registration marks and stamped labels in the margins — applied with restraint so it reads as print/terminal, not chaos.',
      'Mechanical motion: stepped/snap transitions, a monospace marquee ticker, ticking counters, a blinking cursor, subtle glitch/jitter on hover; avoid soft eased fades. Drop the jitter and scanline animation under prefers-reduced-motion.',
      'Stamped metadata system: number every section (01 / 02 — genuinely sequential here), add coordinates, timestamps and version stamps with hairline-ruled labels in the margins to sell the declassified-document feel.',
    ],
    imagePrompts: [
      'high-contrast black-and-white photograph with heavy film grain and halftone dithering, industrial/technical subject, declassified-document feel, registration marks, brutalist print aesthetic',
      'technical schematic or blueprint diagram with monospace annotations and coordinates, a single hazard-orange accent line on a concrete-grey background, engineered and utilitarian',
      'duotone scanned-document texture with scanlines and photocopy artifacts, stamped labels and section numbers, military-terminal readout aesthetic, no soft gradients',
    ],
    source: 'skill',
  },
];
